...
render_io_rows({LastIn, LastOut}) ->
    {{input, In}, {output, Out}} = erlang:statistics(io),
    {
        ?render([
            ?YELLOW,
            ?W("Byte Input", 14), ?W({byte, In - LastIn}, 12),
            ?W("Byte Output", 13), ?W({byte, Out - LastOut}, 12),
            ?W("Total Input", 15), ?W({byte, In}, 17),
            ?W("Total Output", 15), ?W({byte, Out}, 17)
        ]),
        {In, Out}
    }.
...
render_inet_rows(InetList, Num, #inet{type = Type, pages = Pages,
    ...
    {Rows, PortList, _} = lists:foldl(FormatFunc, {[], [], Start},
