...
render_worker(dict, Interval, Pid, TimeRef, RedQ, MemQ, PrintFun) ->
    case erlang:process_info(Pid, dictionary) of
        {dictionary, List} ->
            ...
            Line1 = io_lib:format("erlang:process_info(~p, dictionary). dictionary_len:~p       ~n", [Pid, Len]),
            Line2 =
                case Len of
...
render_worker(stack, Interval, Pid, TimeRef, RedQ, MemQ, PrintFun) ->
    case erlang:process_info(Pid, current_stacktrace) of
        {current_stacktrace, StackTrace}  ->
            ...
            LastLine = render_last_line(),
            {_, Line} =
                lists:foldr(fun({Mod, Func, Arity, Location}, {Nth, Acc}) ->
...
render_process_info(Pid, RegisteredName, GroupLeader, Status, TrapExit, InitialCall,
    ...
    [
        ?render([
            ?GRAY_BG,
            ?W("Meta", 16), ?W("Value", 42),
            ?W("Memory Used", 16), ?W("Value", 12),
            ?W("Grabage Collection", 18), ?W("Value", 12)
            ]),
        ?render([
            ?W("registered_name", 16), ?W(Name, 42),
            ?W("msg_queue_len", 16), ?W2(MessageQueueLenColor, MessageQueueLenStr, 13),
            ?W(" min_bin_vheap_size", 19), ?W({byte, MinBinVHeapSize}, 12),
            ?NEW_LINE,
            ?W("initial_call", 16), ?W(InitialCallStr, 42),
            ?W("heap_size", 16), ?W({byte, HeapSize}, 12),
            ?W("min_heap_size", 18), ?W({byte, MinHeapSize}, 12),
            ?NEW_LINE,
            ?W("group_leader", 16), ?W(GroupLeaderStr, 42),
            ?W("total_heap_size", 16), ?W({byte, TotalHeapSize}, 12),
            ?W("fullsweep_after", 18), ?W({byte, FullSweepAfter}, 12),
            ?NEW_LINE,
            ?W("status", 16), ?W(Status, 42),
            ?W("trap_exit", 16), ?W(TrapExit, 12),
            ?W("minor_gcs", 18), ?W(MinorGcs, 12)
        ])
    ].
...
render_link_monitor(Link, Monitors, MonitoredBy) ->
    ...
    ?render([
        ?W(LinkInfo, 16), ?W(LinkStr, 112), ?NEW_LINE,
        ?W(MonitorInfo, 16), ?W(MonitorsStr, 112), ?NEW_LINE,
        ?W(MonitoredByInfo, 16), ?W(MonitoredByStr, 112)
    ]).
...
render_reduction_memory(Reduction, Memory, ReductionQ, MemoryQ) ->
    {NewRed, NewMem}
        = case queue:len(ReductionQ) >= 20 of
              ...
              false -> {queue:in(Reduction, ReductionQ), queue:in(Memory, MemoryQ)}
          end,
    View =
        [
...
get_menu_title2(info) ->
    [?UNSELECT("Home(H)"), ?SELECT("Process Info(P)"), ?UNSELECT("Messages(M)"),
        ?UNSELECT("Dictionary(D)"), ?UNSELECT("Current Stack(C)"), ?UNSELECT("State(S)")];
get_menu_title2(message) ->
    [?UNSELECT("Home(H)"), ?UNSELECT("Process Info(P)"), ?SELECT("Messages(M)"),
        ?UNSELECT("Dictionary(D)"), ?UNSELECT("Current Stack(C)"), ?UNSELECT("State(S)")];
get_menu_title2(dict) ->
    [?UNSELECT("Home(H)"), ?UNSELECT("Process Info(P)"), ?UNSELECT("Messages(M)"),
        ?SELECT("Dictionary(D)"), ?UNSELECT("Current Stack(C)"), ?UNSELECT("State(S)")];
get_menu_title2(stack) ->
    [?UNSELECT("Home(H)"), ?UNSELECT("Process Info(P)"), ?UNSELECT("Messages(M)"),
        ?UNSELECT("Dictionary(D)"), ?SELECT("Current Stack(C)"), ?UNSELECT("State(S)")];
get_menu_title2(state) ->
    [?UNSELECT("Home(H)"), ?UNSELECT("Process Info(P)"), ?UNSELECT("Messages(M)"),
        ?UNSELECT("Dictionary(D)"), ?UNSELECT("Current Stack(C)"), ?SELECT("State(S)")].
