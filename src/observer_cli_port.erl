...
render_port_info(Port, Id, Name, OsPid, Input,
    Output, Memory, QueueSize, Connected) ->
    QueueSizeColor  =
        case QueueSize > 0 of
            true -> ?RED;
            false -> ?GREEN
    ...
    Title =
        ?render([
            ?GRAY_BG,
            ?W("Attr", 18), ?W("Value", 20),
            ?W("Attr", 18), ?W("Value", 20),
            ?W("Attr", 19), ?W("Value", 21)
            ]),
    Rows =
        ?render([
            ?W("port", 18), ?W(Port, 20),
            ...
            ?W("name", 19), ?W(Name, 21),
            ?NEW_LINE,
            ?W("queue_size", 18), ?W2(QueueSizeColor, QueueSize, 21),
            ?W(" input", 19), ?W({byte, Input}, 20),
            ?W("output", 19), ?W({byte, Output}, 21),
            ?NEW_LINE,
            ?W("connected", 18), ?W(Connected, 20),
            ?W("memory", 18), ?W({byte, Memory}, 20),
            ?W("os_pid", 19), ?W(OsPid, 21)
        ]),
    [Title, Rows].
...
render_link_monitor(Link, Monitors) ->
    ...
    ?render([
        ?W(LinkInfo, 18), ?W(LinkStr, 110), ?NEW_LINE,
        ?W2(?UNDERLINE, MonitorInfo, 19), ?W2(?UNDERLINE, MonitorsStr, 111)
    ]).
...
render_opts(Opts) ->
    ...
    Title =
        ?render([
            ?GRAY_BG,
            ?W("Option", 9), ?W("Value", 6),
            ?W("Option", 14), ?W("Value", 12),
            ?W("Option", 9), ?W("Value", 12),
            ?W("Option", 13), ?W("Value", 8),
            ?W("Option", 9), ?W("Value", 12)
            ]),
    Rows =
        ?render([
            ?W("mode", 9), ?W(Mode, 6),
            ?W("recbuf", 14), ?W({byte, RecBuf}, 12),
            ?W("sndbuf", 9), ?W({byte, SndBuf}, 12),
            ?W("delay_send", 13), ?W(DelaySend, 8),
            ?W("dontroute", 9), ?W(DontRoute, 12),
            ?NEW_LINE,
            ?W("reuseaddr", 9), ?W(ReuseAddr, 6),
            ?W("packet_size", 14), ?W({byte, PacketSize}, 12),
            ?W("buffer", 9), ?W({byte, Buffer}, 12),
            ?W("exit_on_close", 13), ?W(ExitOnClose, 8),
            ?W("priority", 9), ?W(Priority, 12),
            ?NEW_LINE,
            ?W("active", 9), ?W(Active, 6),
            ?W("low_watermark", 14), ?W({byte, LowWatermark}, 12),
            ?W("header", 9), ?W(Header, 12),
            ?W("keepalive", 13), ?W(KeepAlive, 8),
            ?W("linger", 9), ?W(Linger, 12),
            ?NEW_LINE,
            ?W("nodelay", 9), ?W(NoDelay, 6),
            ?W("high_watermark", 14), ?W({byte, HighWatermark}, 12),
            ?W("broadcast", 9), ?W(Broadcast, 12),
            ?W("send_timeout", 13), ?W(SendTimeout, 8),
            ?W("packet", 9), ?W(Packet, 12)
        ]),
    [Title, Rows].
